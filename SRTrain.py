...
def model_conv(batchSize, numNeurons, time_steps, num_features, outputs):
    ...
    x = Convolution1D(128, 2, activation='relu', kernel_regularizer=regularizers.l2(regVal), bias_regularizer=regularizers.l2(regVal))(x_in)
    x = BatchNormalization()(x)
    x = Dropout(0.4)(x)
    x = MaxPooling1D(pool_size=2)(x)
    x = Convolution1D(64, 2, activation='relu', kernel_regularizer=regularizers.l2(regVal), bias_regularizer=regularizers.l2(regVal))(x)
    x = BatchNormalization()(x)
    x = Dropout(0.4)(x)
    x = MaxPooling1D(pool_size=2)(x)
    x = Convolution1D(32, 2, activation='relu', kernel_regularizer=regularizers.l2(regVal), bias_regularizer=regularizers.l2(regVal))(x)
    x = BatchNormalization()(x)
    x = Dropout(0.4)(x)
    x = MaxPooling1D(pool_size=2)(x)
    x = Flatten()(x)
